...
display_url = 'https://display.musinsa.com/display/brands/%s/goods?page=%d'


def _brandsCrawler(page_num, brand):
    soup = BeautifulSoup(requests.get(display_url %
                         (brand.url, page_num)).text, 'html.parser')
    ...
    for price, image, product_id, name, url in zip(prices, images, product_ids, names, urls):
        ...
        try:
            ...
            update_products.apppend(product)
        except:
            product = Products(id=product_id, name=name, brand=brand, current=history,
                               lowest=history, highest=history, image=image, created_date=datetime)
            new_products.append(product)
    History.objects.bulk_create(histories)
    Products.objects.bulk_create(new_products)
    Products.objects.bulk_update(update_products)
